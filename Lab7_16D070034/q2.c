...
unsigned long avgVal=0, initVal=0, adcValue = 0, temp = 0;
unsigned char serial_data;
unsigned char data_save_high;
unsigned char data_save_low;
unsigned char  voltage[2] ;
unsigned int count=0,avgcnt = 0, i=0;
float fweight=0;
...
void main(void)
{
		...
		while(1)
	{
		...
		CS_BAR = 1;    
	adcVal = (data_save_high << 2) + (data_save_low  >> 6);		// save the 10 bits in one variable
		//adcVal = (tempadcVal >> 6) ;
    avgVal = 0.0209*adcVal*3300/1024;	
...
void timer0_ISR (void) interrupt 1
{
   ...
   //avgVal += adcValue ;
	...
	//	delay_ms(100) ;
		LCD_Ready();
		LCD_CmdWrite(0x80);
		LCD_StringWrite("Distance: ", 10);
		if (temp > 1400 ){
		LCD_StringWrite("5 cm or more", 4);
		}
		else if ((temp < 1400 ) && (temp  > 1300)){
		LCD_StringWrite("4 cm", 4);
		}
		else {
			LCD_StringWrite(voltage, 2) ;
			LCD_StringWrite(" mm", 3);
		}
	/*	else if ((temp < 1156 ) && (temp  > 1050)){
		LCD_StringWrite("2.5 cm", 4);
		}
		else if ((temp < 1050 ) && (temp  > 850)){
		LCD_StringWrite("2 cm", 4);
		}
		else if ((temp < 850 ) && (temp  > 450)){
		LCD_StringWrite("1.5 cm", 4);
		}
		else if ((temp < 850 ) && (temp  > 450)){
		LCD_StringWrite("1.5 cm", 4);
		}*/
...
void SPI_Init()
{
	...
	SPCON |= 0x20;               	 	// P1.1(SSBAR) is available as standard I/O pin  SPEN
